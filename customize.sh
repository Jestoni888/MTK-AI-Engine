...
ui_print "=================================="
ui_print "     MTK AI Engine 0.0.0.02"
...
ui_print "  ✅🔋⚡AI MODES IN GAMING ↔️ NORMAL USE ↔️ POWERSAVE (SCREEN-OFF)"
ui_print "  ✅auto cpuset configure during gaming & non-gaming"
ui_print "  ✅auto thermal disabled during gaming 🔥 also enabled thermal during normal use ❄️"
ui_print "  ✅auto cpu frequency adjustment during gaming(100%🔒) & non-gaming(based on temperature)"
ui_print "  ✅auto performance mode in gaming (cpu governor governor)"
ui_print "  ✅Auto PPM policies during gaming mode & non-gaming mode"
#ui_print "  ✅Autokill during gaming is added back in this version safest & cleanest mode"
#ui_print "  ✅Zram tuning when gaming (swappiness=10) & non-gaming (swappiness=60)"
ui_print "  ✅Lot of props added explore inside"
ui_print "  💡see logs in MTK_AI_Engine.log for more details & info"
...
EOF
  ...
  ui_print "✅ Created $GAME_LIST_FILE (see game_list.txt in sdcard add your game package name if not on the lists.
  NOTE: you need to restart the device if you plan to add the game package)"
else
  ui_print "⚠️ $GAME_LIST_FILE already exists (see game_list.txt in sdcard add your game package name if not on the lists.
  NOTE: you need to restart the device if you plan to add the game package)"
fi
...
com.android.carrierconfig.overlay.product
...
com.android.rkpdapp
EOF
#  chmod 0666 "$EXCLUDE_FILE"
#  ui_print "✅ Created $EXCLUDE_FILE (see exclude_apps.txt in sdcard add your app package name if you don't want it to be killed during gaming)"
#else
#  ui_print "⚠️ $EXCLUDE_FILE already exists (see exclude_apps.txt in sdcard add your app package name if you don't want it to be killed during gaming)"
#fi
...
if [ ! -f "$LOG_FILE" ]; then
  ...
  ui_print "⚠️ $LOG_FILE already exists"
fi

ui_print "🎮 Game Booster installed. Reboot to enable service."

# Set permissions for COOLER
set_perm_recursive $MODPATH/system/etc/cooler 0 0 0755 0644
ui_print "- Cooler script added"
ui_print "- feel free to copy my modules, I'm gonna share my ideas💡 to everyone! ENJOY 😉"
ui_print "- Done!"
