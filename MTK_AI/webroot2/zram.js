...
(function() {
    ...
    async function applyZram(algo) {
        ...
        if (statsBox) statsBox.textContent = 'Resetting & applying...';
        ...
        try {
            await execFn(`
                ...
                echo 1 > "/sys/block/$ZNAME/reset"                echo "${algo}" > "/sys/block/$ZNAME/comp_algorithm"
                echo "${sizeStr}" > "/sys/block/$ZNAME/disksize"
                mkswap "$ZDEV" > /dev/null 2>&1
                swapon -p 100 "$ZDEV" 2>/dev/null || swapon "$ZDEV"
                echo ${swappiness} > /proc/sys/vm/swappiness
            `);

            await execFn(`mkdir -p ${CONFIG_DIR} && cat > ${CONFIG_FILE} <<EOF
SIZE=${sizeStr}
ALGO=${algo}
SWAP=${swappiness}
EOF`);
            ...
            if (statsBox) statsBox.textContent = '✅ Applied successfully!';
            ...
            if (statsBox) statsBox.textContent = '❌ Failed. Check root.';
