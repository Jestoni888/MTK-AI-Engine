...
const execCmd = async function(cmd, timeout = 8000) {
 ...
 function formatChangelog(md) {
     ...
     for (let line of lines) {
         ...
         if (line.startsWith('--- version') && line.endsWith('---')) {
             ...
             html += `<div style="color:#4a9eff;font-weight:bold;font-size:13px;margin-top:12px;margin-bottom:6px;border-bottom:1px solid #2a3152;padding-bottom:4px;">🚀 Version ${ver}</div>`;
         } else if (line.startsWith('•')) {
             html += `<div style="margin-left:8px;margin-bottom:5px;color:#d1d5db;display:flex;align-items:flex-start;gap:6px;"><span style="color:#8b92b4;">•</span><span>${line.substring(1).trim()}</span></div>`;
         } else {
             html += `<div style="color:#d1d5db;margin-bottom:5px;">${line}</div>`;
         }
     }
 ...
 function showTopBanner() {
     ...
     banner.innerHTML = `
         ...
         <div id="banner-body" style="padding: 0 16px; max-height: 0; overflow: hidden; transition: all 0.4s ease-out;">
             <div style="color: #8b92b4; font-size: 12px; margin-bottom: 8px; font-weight: 600; padding-top: 12px;">Changelog (Latest):</div>
             <div id="banner-changelog-content" style="color: #e0e0e0; font-size: 12px; line-height: 1.5; max-height: 300px; overflow-y: auto; margin-bottom: 16px; padding: 8px; background: #0a0c10; border-radius: 8px; border: 1px solid #2a3152;">
                 ⏳ Loading changelog...
             </div>
             <div style="display: flex; gap: 12px; padding-bottom: 16px;">
                 <button id="banner-close-btn" style="width: 100%; padding: 12px; background: #2a3152; color: #fff; border: none; border-radius: 10px; font-size: 14px; font-weight: 600; cursor: pointer;">Close</button>
             </div>
         </div>
     `;
 ...
 // 🌐 Fetch changelog: WebView fetch first (reliable TLS), shell as last resort
 ...
 function showUpdateModal(data) {
     ...
     modal.style.cssText = 'background:linear-gradient(135deg,#1a1f3a,#151b2d);border-radius:16px;padding:24px;max-width:480px;width:90%;color:#fff;border:2px solid #4a9eff;max-height:90vh;overflow-y:auto;';
     ...
     const changelogHTML = `
         <div style="margin-top:16px;padding:12px;background:#0a0c10;border-radius:8px;border:1px solid #2a3152;max-height:250px;overflow-y:auto;">
             <div style="color:#8b92b4;font-size:12px;margin-bottom:8px;font-weight:600;">Changelog (Latest):</div>
             <div id="changelog-content" style="color:#e0e0e0;font-size:12px;font-family:sans-serif;line-height:1.5;">⏳ Loading changelog...</div>
         </div>`;
     modal.innerHTML = `
     ...
     document.getElementById('dl-btn').onclick = async () => {
         ...
         const poll = setInterval(async () => {
             ...
             if (isDone) {
                 ...
                 await execCmd(`su -c '${BUSYBOX} rm -f "${STATUS_FILE}"'`, 2000);
 ...
 async function checkForUpdates(refresh = false) {
     ...
     // Otherwise, use original modal logic (with download button)
 ...
 if (updateContainer) {
     ...
     updateContainer.addEventListener('click', async (e) => {
         ...
         if (isAutoUpdate) {
             showTopBanner();
         } else if (statusData?.update_available) {
             showUpdateModal(statusData);
         } else {
