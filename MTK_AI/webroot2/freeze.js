...
// ✅ NEW: Update only the specific app row in-place
function updateAppRowUI(pkg, isNowFrozen) {
    ...
    // Update status label (2nd child > 2nd div)
    const infoDiv = appEl.querySelector('div:nth-child(2)');
    if (infoDiv) {
        ...
        for (const line of lines) {
            ...
            appEl.innerHTML = `
                ...
                <div style="flex:1;min-width:0;">
                    ...
                    <div style="color:#555;font-size:10px;font-family:monospace;margin-top:1px;">${pkg}</div>
                </div>
        ...
        listEl.querySelectorAll('.freeze-app-toggle').forEach(btn => {
            btn.onclick = async (e) => {
                ...
                try {
                    ...
                    // ✅ IN-PLACE UPDATE — no full modal refresh
...
async function toggleAllApps(freeze) {
    ...
    try {
        ...
        for (const app of detectedApps) {
            ...
            // ✅ Update each row in-place during bulk operation
