(function() {
    ...
    const state = {
        ...
        lastPathCheck: 0,
        pathStatus: null,
        consoleLoaded: false,  // ✅ NEW: Track if console has been initialized
        deviceInfo: {
            name: 'Detecting...',
            codename: '...',
            android: '...',
    ...
    // ... [KEEP YOUR EXISTING PERFORMANCE_PATHS & CRITICAL_PATHS UNCHANGED] ...
    const PERFORMANCE_PATHS = {
        ...
        cpu: [
            ...
            '/sys/devices/system/cpu/cpufreq/policy0/scaling_governor',            '/sys/devices/system/cpu/cpufreq/policy0/scaling_available_frequencies',
    ...
    const CRITICAL_PATHS = {
        ...
        thermal: ['/sys/class/thermal/thermal_zone0/temp'],        power: ['/sys/class/power_supply/battery/capacity'],
        logcat: ['/dev/log/main']
    };
    ...
    function injectTerminalStyles() {
        ...
        style.textContent = `
            .terminal-container { 
                ...
                cursor: pointer !important; /* ✅ Visual cue: clickable */
            ...
            .terminal-container .term-line { display: flex; margin: 1px 0; align-items: flex-start; }            .terminal-container .term-label { width: 65px; flex-shrink: 0; color: #3bc7e5; font-weight: bold; font-size: 10px; }
                ...
                readable.forEach(item => {
                    ...
                    if (shortPath === 'mode' && cat === 'thermal') {
                        if (item.value?.trim() === 'disabled') {
                            ...
                            statusClass = 't-bright-cyan';                            statusText = '[ENABLED]';
                        }
                    }
    ...
    // ✅ NEW: Load console on user interaction
    async function loadConsoleOnDemand() {
        if (state.consoleLoaded) return; // Already loaded
        ...
        try {
            ...
            state.consoleLoaded = true; // ✅ Mark as loaded
    ...
    async function checkCriticalPaths() {
        const results = { timestamp: Date.now(), categories: {}, summary: { total: 0, accessible: 0, restricted: 0, missing: 0 } };        for (const [category, paths] of Object.entries(CRITICAL_PATHS)) {
    ...
    async function parseCpuUsage() {
        try {
            ...
            if (totalDiff <= 0) return state.cpuUsage;            return Math.max(0, Math.min(100, Math.round((1 - (idleDiff / totalDiff)) * 100)));
    ...
    function setupEventListeners() {
        ...
        // ✅ Console click-to-load handler
        const consoleEl = document.getElementById('inline-console');
        if (consoleEl) {
            consoleEl.addEventListener('click', (e) => {
                // Prevent if clicking refresh button
                if (e.target.closest('#inline-refresh-btn')) return;
                loadConsoleOnDemand();
            });
        }
        
        // ✅ Inline console refresh button
        document.getElementById('inline-refresh-btn')?.addEventListener('click', async () => {
            ...
            // Auto-load console first if not already loaded
            if (!state.consoleLoaded) {
                await loadConsoleOnDemand();
            }
