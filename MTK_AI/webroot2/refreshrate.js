...
async function loadAvailableModes() {
    try {
        ...
        availableModes = lines.map(line => {
            ...
            return {
                // Force Auto to use ID -1 for the SurfaceFlinger command
                id: isAuto ? '-1' : id.trim(), 
                label: hzMatch ? hzMatch[1] + ' Hz' : (label?.trim() || 'Mode ' + id)
            };
        });

        // Ensure 'Auto' (-1) is always present and at the top
        if (!availableModes.some(m => m.id === '-1')) {
            availableModes.unshift({ id: '-1', label: 'Auto' });
        } else {
            // Move Auto to the top if it exists
            const autoIndex = availableModes.findIndex(m => m.id === '-1');
            if (autoIndex > 0) {
                const [autoMode] = availableModes.splice(autoIndex, 1);
                availableModes.unshift(autoMode);
            }
        }
    ...
    // Fallback modes if script fails or returns nothing
...
function updateDisplay(modeId) {
    ...
    const text = mode ? mode.label : (modeId === '-1' ? 'Auto' : `${modeId} Hz`);
...
async function applyRefreshMode(modeId) {
    const modal = document.getElementById('refresh-rate-modal');        
    if (modal) modal.remove();

    try {
        // 1. Apply SurfaceFlinger command (using -1 for Auto)
        await execFn(`su -c "service call SurfaceFlinger 1035 i32 ${modeId} >/dev/null 2>&1"`);
