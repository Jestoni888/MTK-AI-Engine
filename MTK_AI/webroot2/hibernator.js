...
const CRITICAL_APPS = [
    'com.android.systemui', 'com.android.phone', 'com.android.stk',
    'com.google.android.gms', 'com.google.android.gsf',
    'com.google.android.packageinstaller', 'com.android.settings',
    'com.google.android.setupwizard', 'com.android.launcher3',
    'com.google.android.inputmethod.latin', 'android',
    'com.android.providers.settings', 'com.android.providers.downloads',
    'com.android.providers.media.module', 'com.android.externalstorage',
    'com.android.documentsui', 'com.android.permissioncontroller',
    'com.android.vending'
];
...
async function getAllPackages() {
    ...
    try {
        if (typeof ksu !== 'undefined' && typeof ksu.listPackages === 'function') {
            pkgs = normalizePkgList(await Promise.resolve(ksu.listPackages('all')));
        }
    } catch (e) {}
    if (!pkgs.length) {
        try { pkgs = normalizePkgList(await execFn('pm list packages 2>/dev/null', 5000)); } catch (e) {}
    }
    return pkgs;
}
...
async function loadCleanerToggle() {
    try {
        const exists = await execFn(`test -f ${ENABLE_CLEANER_FILE} && echo "yes" || echo "no"`);
        const toggle = document.getElementById('enable-cleaner-toggle');
...
async function getAppRunningInfo(pkg) {
    try {
        ...
        if (lines.length > 0) {
            ...
            return { isRunning: true, pid: pid, processCount: lines.length, memInfo: memInfo };
        }
...
function filterPackages() {
    ...
    const filtered = installedPackages.filter(pkg => {
        const appName = getAppName(pkg).toLowerCase();
        return appName.includes(q) || pkg.toLowerCase().includes(q);
    });
    renderPackages(filtered);
}
...
async function killAllRunningNow() {
    ...
    for (const pkg of runningApps) {
        try {
            await execFn(`am force-stop ${pkg}`);
            killedCount++;
            statusEl.textContent = `☠️ Killed ${killedCount}/${runningApps.length}: ${getAppName(pkg)}`;
