...
// OPTIMIZATION: Bind UI immediately, load data in background to prevent blocking
async function init() {
    bindClickHandler(); 
    Promise.all([loadConfig(), fetchSchedulers()]).catch(e => console.warn('Init background tasks failed:', e));
}

async function fetchSchedulers() {
    try {
        // OPTIMIZATION: Removed redundant 'su -c' since execFn already runs as root
...
// OPTIMIZATION: Use grep -H to read files in a single fast pass instead of a slow while-read-cat loop
    ...
    try {
        ...
        if (result && result.trim()) {
            ...
            lines.forEach(line => {
                ...
                // Format "path:content" to "path => content"
...
function showIOModal() {
    ...
    // OPTIMIZATION: Defer log update so the modal appears instantly without waiting for shell execution
...
// OPTIMIZATION: Execute a single shell script to apply all tweaks at once instead of looping in JS
