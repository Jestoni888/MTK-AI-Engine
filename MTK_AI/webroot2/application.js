...
// Toggle game list - OPTIMIZED (No Reload)
async function toggleGameList(pkg) {
    try {
        ...
        if (app.isInGameList) {
            ...
            showStatus('Removed: ' + app.label, '#FF453A');
        } else {
            // Add to game list
            gameList.push(pkg);
            await execFn(`echo "${pkg}" >> ${GAMELIST_FILE}`);
            showStatus('Added: ' + app.label, '#32D74B');
        }
        ...
        // Re-render UI instantly without shell commands
        renderAppList(allApps);
...
// Load app config
async function loadAppConfig(pkg) {    try {
        ...
        if (eemResult.trim()) {
            ...
            if (slider) {                slider.value = eemResult.trim();
