...
<div style="margin-bottom:12px;">
    ...
    <input type="range" id="config-downscale" min="10" max="100" value="100" step="5" 
           style="width:100%;accent-color:#FF9F0A;" 
           oninput="document.getElementById('downscale-val').textContent=(this.value/100).toFixed(1)+'x'">
    <div style="text-align:center;color:#FF9F0A;font-size:11px;margin-top:4px;">
        <span id="downscale-val">1.0x</span> <small style="color:#666">(0.1x = 10% render resolution)</small>
    </div>
</div>
                     </div>
                 </div>
...
async function loadAppConfig(pkg) {    
    try {
        ...
        // Downscale Factor (stored as integer 10-100, represents 0.1-1.0)
const downscaleResult = await execFn(`cat ${PERAPP_DIR}/${pkg}.downscale 2>/dev/null`);
if (downscaleResult.trim()) {
    const slider = document.getElementById('config-downscale');
    const display = document.getElementById('downscale-val');
    if (slider) {
        slider.value = downscaleResult.trim();
        if (display) display.textContent = (downscaleResult.trim() / 100).toFixed(1) + 'x';
    }
}
...
async function saveAppConfig(pkg) {
    try {
        ...
        // ➕ Downscale: Save + Execute Immediately (1.0 → 0.1 range)
const downscaleRaw = document.getElementById('config-downscale').value;
const downscaleDecimal = (parseInt(downscaleRaw) / 100).toFixed(1); // "0.7", "0.5", etc.

// Save to file for persistence (store as integer 10-100)
await execFn(`mkdir -p ${PERAPP_DIR} && echo "${downscaleRaw}" > ${PERAPP_DIR}/${pkg}.downscale`);

//Force stop before setup
await execFn(`am force-stop ${pkg}`);

// 🔥 EXECUTE IMMEDIATELY via game service
await execFn(`cmd game set --downscale ${downscaleDecimal} ${pkg}`);

console.log(`🎮 Downscale applied: ${pkg} @ ${downscaleDecimal}x`);
