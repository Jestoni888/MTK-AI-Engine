...
async function openExternal(url) {
    ...
    if (url.startsWith('mailto:')) {
        const cmd = `am start -a android.intent.action.SENDTO -d "${url}" 2>/dev/null`;
...
function openModal() { 
    const o = document.getElementById('profile-modal-overlay'); 
...
window.ProfileModal = { 
    ...
    toggle: () => { 
        const o = document.getElementById('profile-modal-overlay'); 
        if (o?.style.display === 'flex') closeModal(); 
        else openModal(); 
    } 
};
