...
const scriptPaths = {
    ...
    disable_log: { on: null, off: null, isDaemon: false, critical: false },
    app_priority: { on: null, off: null, isDaemon: false, critical: false },
    compiler: { on: null, off: null, isDaemon: false, critical: false },
...
const toggleConfig = {
    ...
    selinux_mode: { 
        ...
        desc: "Disable mandatory access control enforcement", 
        group: "System",
        warning: "⚠️ Breaks SafetyNet/Play Integrity & banking apps. Use at your own risk."
    },
    fast_reboot: { 
...
function showStatus(msg, isError = false) {
    ...
    const txt = document.getElementById('mtk-ai-status-text');    if (el && txt) {
            ...
            html += `
              <div class="setting-item">
                  ...
                  <div class="setting-content">
                      ...
                      <div id="cpu-freq-display" style="margin-top:8px;"></div>                  </div>
              </div>`;
        }
        html += `</div>`;
    }
...
async function applyTouchActiveFreq(e) {
    ...
    try {
        await execFn(`echo "${val}" > /sdcard/MTK_AI_Engine/manual_touch_active_freq.txt 2>/dev/null`, 2000);        showStatus(`✅ Touch active frequency set to ${val}%`);
...
async function applyCPUFrequencyPercent(percent) {
    ...
    try {
        ...
        for (const { cpu, scalingMaxPath } of cpuPaths) {
            const hardwareMax = maxFreqs[cpu] || 2000000;
...
// ✅ NEW: Load tap to rotate state
async function loadTapToRotateState() {    try {
        const rotationState = await execFn('settings get secure accelerometer_rotation', 1000);
        const isEnabled = rotationState.trim() === '0'; // If 0, then "disable tap to rotate" is ON
        const sw = document.getElementById('sw-disable_tap_to_rotate');
        if (sw) {
            sw.checked = isEnabled;
        }
    } catch (e) {
        console.warn('Failed to load tap to rotate state:', e);
    }
}

// ✅ NEW: Apply tap to rotate toggle
async function applyTapToRotateToggle(isEnabled) {
    try {
        if (isEnabled) {
            // Disable tap to rotate / hide rotation button
            await execFn('settings put secure accelerometer_rotation 0', 2000);
            await execFn('settings put secure show_rotation_suggestions 0', 2000);
            await execFn('settings put system accelerometer_rotation 0', 2000);
        } else {
            // Enable tap to rotate / show rotation button
            await execFn('settings put secure accelerometer_rotation 1', 2000);
            await execFn('settings put secure show_rotation_suggestions 1', 2000);
            await execFn('settings put system accelerometer_rotation 1', 2000);
        }
        showStatus(`✅ Tap to Rotate ${isEnabled ? 'disabled' : 'enabled'}`);
    } catch (err) {
        showStatus('⚠️ Failed to apply rotation setting', true);
    }
}
...
async function loadAllStates() {
    ...
    for (const key of keys) {
        ...
        try {
            ...
            if (sw) {
                ...
                } else if (key === 'disable_tap_to_rotate') {
                    // Special handler for tap to rotate
                    sw.addEventListener('change', async (e) => {                        await applyTapToRotateToggle(e.target.checked);
                        await saveStateForKey(key, e.target.checked ? '1' : '0');
                    });
    ...
    await loadTapToRotateState(); // ✅ Load tap to rotate state
}
...
async function applyToggle(key, isEnabled) {
    ...
    // ✅ Custom logic for selinux_mode toggle
    if (key === 'selinux_mode') {
        const flagFile = `${CFG_DIR}/selinux_permissive`;
        if (isEnabled) {
            showStatus('🛡️ Setting SELinux to Permissive...');
            try {
                await execFn('su -c "setenforce 0" 2>/dev/null', 2000);
                await execFn(`su -c "echo '1' > ${flagFile} && chmod 644 ${flagFile}" 2>/dev/null`, 1000);
                await saveStateForKey(key, '1');
                showStatus('✅ SELinux set to Permissive');
            } catch (e) { showStatus('❌ Failed: setenforce 0 blocked by kernel/ROM', true); }
        } else {
            showStatus('🔒 Setting SELinux to Enforcing...');
            try {
                await execFn('su -c "setenforce 1" 2>/dev/null', 2000);
                await execFn(`su -c "rm -f ${flagFile}" 2>/dev/null`, 1000);
                await saveStateForKey(key, '0');
                showStatus('✅ SELinux set to Enforcing');
            } catch (e) { showStatus('❌ Failed: setenforce 1 blocked by kernel/ROM', true); }
        }
    ...
    // ✅ Custom logic for fast_reboot toggle
    if (key === 'fast_reboot') {
        const flagFile = `${CFG_DIR}/fast_reboot_enabled`;
        if (isEnabled) {
            showStatus('⚡ Fast reboot mode enabled (applies on next reboot)');
            try {
                // Set persistent property (optional cache)
                await execFn('su -c "setprop persist.sys.mtk.fast_boot 1" 2>/dev/null', 1500);
                // Create flag file for service.sh to read at boot
                await execFn(`su -c "echo '1' > ${flagFile} && chmod 644 ${flagFile}" 2>/dev/null`, 1500);
                await saveStateForKey(key, '1');
            } catch (e) { 
                showStatus('❌ Failed to enable fast reboot', true); 
            }
        } else {
            showStatus('🔌 Fast reboot mode disabled');
            try {
                await execFn('su -c "setprop persist.sys.mtk.fast_boot 0" 2>/dev/null', 1500);
                await execFn(`su -c "rm -f ${flagFile}" 2>/dev/null`, 1000);
                await saveStateForKey(key, '0');
            } catch (e) { 
                showStatus('❌ Failed to disable fast reboot', true); 
            }
        }
        if (sw) sw.disabled = false;
        await restartService(); // Restart service to sync state (no reboot triggered)
        return;
    }
    ...
    if (key === 'disable_system_prop') {
        if (isEnabled) {
            ...
            try {
                ...
                await saveStateForKey(key, '1');
                await restartService();                showStatus('✅ system.prop removed successfully.');
                setTimeout(() => {
