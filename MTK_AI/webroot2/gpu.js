...
const CONFIG_DIR = '/sdcard/MTK_AI_Engine';
 ...
 // ✅ Write -1 to config file to indicate unlocked state
 await execFn(`mkdir -p ${CONFIG_DIR} && echo '-1' > ${CONFIG_FILE} 2>/dev/null`);
 ...
 async function applyAllGpuSettings(applyBtn, statusEl) {
     ...
     try {
         ...
         if (isFreqLocked) {
             ...
             await saveToSdCard(-1); // ✅ Write -1 to config file to indicate dynamic scaling
         }
         // Update card display
         updateCardDisplay();
         // Success feedback
