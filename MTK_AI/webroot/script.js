...
const SYSCTL_SCAN_CONFIG = {
    ...
    directories: ['vm', 'net/core', 'net/ipv4', 'kernel', 'debug'],
    // Specific files to include outside folders (rare but possible)
    specificFiles: [], 
    // Patterns to IGNORE (Safety & Noise reduction)
    ignorePatterns: [
    ...
    // Max items to render per category to prevent UI freeze
    maxItemsPerCategory: 30 
};

let dynamicSysctlData = [];

async function initDynamicSysctl() {
    const container = document.getElementById('sysctl-controls-container');
    const status = document.getElementById('sysctl-status');
    
    if (!container) return;

    status.textContent = "Scanning kernel filesystem...";
    status.style.color = "#FF9F0A";
    container.innerHTML = '<div style="padding:20px; text-align:center;">🔍 Scanning /proc/sys/...</div>';
    ...
    try {
        ...
        for (const dir of SYSCTL_SCAN_CONFIG.directories) {
            ...
            for (const file of files) {
                ...
                const key = `${dir}.${file}`;
                const path = `${fullPath}/${file}`;                const safeKey = key.replace(/\./g, '_').replace(/\//g, '_');
                const category = dir.split('/')[0].toUpperCase(); // e.g., "VM", "NET"

                // Check if writable (Critical for safety)
                // We try to see if we can write to it. If "Read-only file system", skip.
                // Optimization: Just try to cat first. If it exists, we assume writable for UI, handle error on apply.
                
                let currentValue = await exec(`cat "${path}" 2>/dev/null`);
                currentValue = currentValue.trim();

                // Skip if empty or extremely long (binary data)
                if (!currentValue || currentValue.length > 50) continue;
                ...
                const finalValue = savedValue ? savedValue : currentValue;

                // Determine input type (Number vs String)
                const isNumber = /^-?\d+$/.test(currentValue);
                const min = isNumber ? Math.floor(parseFloat(currentValue) * 0.5) : 0;
                const max = isNumber ? Math.ceil(parseFloat(currentValue) * 2) : 100;
                const step = isNumber ? 1 : 1;

                dynamicSysctlData.push({
                    key, path, safeKey, category,
                    currentValue, finalValue,
                    isNumber, min, max, step,
                    desc: `Dynamic parameter: ${key}`
                });
            }
        }

        // Sort by Category
        dynamicSysctlData.sort((a, b) => a.category.localeCompare(b.category));

        renderDynamicSysctlUI();

        status.textContent = `Found ${dynamicSysctlData.length} tunable parameters`;
        status.style.color = "#32D74B";

    } catch (err) {
        console.error("Dynamic Scan Error:", err);
        container.innerHTML = `<div style="color:#ff453a; text-align:center;">Scan Failed.<br>${err.message}</div>`;
...
function renderDynamicSysctlUI() {
    ...
    if (dynamicSysctlData.length === 0) {
        container.innerHTML = '<div style="text-align:center; color:#666; padding:20px;">No dynamic parameters found or all filtered out.</div>';
        return;
    }
    ...
    for (const item of dynamicSysctlData) {
        ...
        // Limit items per category to prevent freezing
        countInCat++;
        if (countInCat > SYSCTL_SCAN_CONFIG.maxItemsPerCategory) {
            if (countInCat === SYSCTL_SCAN_CONFIG.maxItemsPerCategory + 1) {
                html += `<div style="font-size:11px; color:#666; font-style:italic; padding:5px;">...and more hidden to save performance</div>`;
            }
            continue;
        }

        const sliderHtml = item.isNumber ? `
            <input type="range" 
        ...
        ` : `
            <input type="text" 
                ...
                value="${item.finalValue}"
                onchange="updateSysctlDisplay('${item.safeKey}', this.value)"                style="width: 100%; padding: 8px; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.2); color: white; border-radius: 6px;">
        `;
        ...
        <div style="margin-bottom: 15px; padding-bottom: 15px; border-bottom: 1px solid rgba(255,255,255,0.05);">
            ...
            <div style="font-size:10px; color:#888; margin-bottom:8px;">Current: ${item.currentValue} | Saved: ${item.savedValue ? item.savedValue : 'None'}</div>
