...
async function initDynamicSysctl() {
    ...
    status.textContent = "Scanning & verifying permissions...";
    status.style.color = "#FF9F0A";
    container.innerHTML = '<div style="padding:20px; text-align:center;">🔍 Scanning /proc/sys/ (Strict Mode)...<br><span style="font-size:10px; color:#666">Verifying write access by reading back values</span></div>';
    ...
    try {
        ...
        for (const dir of SYSCTL_SCAN_CONFIG.directories) {
            ...
            for (const file of files) {
                ...
                const category = dir.split('/')[0].toUpperCase();

                // 1. Get Original Value
                let originalValue = await exec(`cat "${path}" 2>/dev/null`);
                originalValue = originalValue.trim();

                if (!originalValue || originalValue.length > 50) continue;

                // --- 🛡️ STRICT WRITABILITY CHECK ---
                // Step A: Try to write the SAME value back
                await exec(`echo "${originalValue}" > "${path}" 2>&1`);
                
                // Step B: Read it back immediately
                let newValue = await exec(`cat "${path}" 2>/dev/null`);
                newValue = newValue.trim();

                // Step C: Compare. If they don't match, it's Read-Only (or the kernel modified it instantly)
                // We also check if the write command returned a known error string just in case
                if (newValue !== originalValue) {
                    // Value changed unexpectedly or reverted -> Treat as Read-Only/Safe Skip
                    continue; 
                }
                
                // Additional Safety: Explicitly check for common read-only paths by name
                if (key.includes("osrelease") || key.includes("ostype") || key.includes("version")) {
                     continue; // Hard skip these known info-only files
                }
                // ---------------------------------------
        ...
        const count = dynamicSysctlData.length;
        status.textContent = `Found ${count} writable parameters`;
...
function renderDynamicSysctlUI() {
    ...
    if (dynamicSysctlData.length === 0) {
        container.innerHTML = '<div style="text-align:center; color:#666; padding:20px;">No dynamic parameters found.</div>';
        return;
    }
    ...
    for (const item of dynamicSysctlData) {
        ...
        // Limit items to prevent freeze
        countInCat++;
        if (countInCat > 15) {
            if (countInCat === 16) {
                html += `<div style="font-size:11px; color:#666; font-style:italic; padding:5px;">...hidden to save performance</div>`;
            }
            continue;
        }

        // 2. Determine Input Type (Slider vs Text)
        const inputHtml = item.isNumber ? `
            <input type="range" 
        ...
        // 3. FIX: Display Saved Value correctly here 👇
        const savedDisplay = item.savedValue ? item.savedValue : 'None';
        const savedColor = item.savedValue ? '#32D74B' : '#666'; // Green if saved, Grey if not
