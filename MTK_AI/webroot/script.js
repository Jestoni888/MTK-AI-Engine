...
const SYSCTL_SCAN_CONFIG = {
    ...
    directories: ['fs', 'vm', 'net/core', 'net/ipv4', 'kernel', 'debug'],
...
async function initDynamicSysctl() {
    ...
    container.innerHTML = '<div style="padding:20px; text-align:center;">🔍 Scanning...<br><span style="font-size:10px; color:#666">Verifying write access by reading back values</span></div>';
...
function renderDynamicSysctlUI() {
    ...
    for (const item of dynamicSysctlData) {
        ...
        countInCat++;
        if (countInCat > 50) {
            if (countInCat === 51) html += `<div style="font-size:11px; color:#666; font-style:italic; padding:5px;">...hidden to save performance</div>`;
            continue;
        }
