...
document.addEventListener('DOMContentLoaded', loadSearchCache);
...
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => {
        ...
        if (slider) {
            slider.addEventListener('input', handleGpuSliderInput);
        }
...
renderGameKillerList(pkgs, saved)

} // End of init function

const GK_FILE = "/sdcard/MTK_AI_Engine/gamekiller_list.txt";
let gameKillerLoaded = false;

// Toggle section visibility (like your other sections)
function toggleGameKillerSection() {
    const content = document.getElementById('gamekiller-content');
    if (content.style.display === 'none') {
        content.style.display = 'block';
        // Load only on first expand
        if (!gameKillerLoaded) {
            loadGameKillerApps();
            gameKillerLoaded = true;
        }
    } else {
...
// Load apps asynchronously (non-blocking)
async function loadGameKillerApps() {
    const content = document.getElementById('gamekiller-content');
    content.innerHTML = '<div class="list-item"><div class="item-desc">⏳ Loading...</div></div>';
    
    try {
        // Load saved list
        let saved = [];
        try {
            const raw = await exec(`cat "${GK_FILE}" 2>/dev/null`);
            saved = raw.split('\n').filter(l => l.trim() && !l.startsWith('#'));
        } catch (e) {}

        // Load packages (defer to next tick to avoid blocking)
        setTimeout(async () => {
            try {
                const raw = await exec('pm list packages -3 2>/dev/null');
                const pkgs = raw.split('\n')
                    .map(l => l.replace('package:', '').trim())
                    .filter(l => l);
                
                renderGameKillerList(pkgs, saved);
            } catch (e) {                content.innerHTML = '<div class="list-item"><div class="item-desc">❌ Failed to load apps</div></div>';
            }
        }, 100);
        
    } catch (e) {
        content.innerHTML = '<div class="list-item"><div class="item-desc">❌ Error</div></div>';
    }
}

// Render the list
function renderGameKillerList(pkgs, saved) {
    const content = document.getElementById('gamekiller-content');
    let html = '';
    
    // Limit to 100 for speed
    pkgs.slice(0, 100).forEach(pkg => {
        const checked = saved.includes(pkg) ? 'checked' : '';
        html += `
        <div class="list-item">
            <div class="item-desc" style="flex:1; min-width:0;">
                <span style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${pkg}</span>
            </div>
            <label class="ios-switch">
                <input type="checkbox" ${checked} onchange="toggleGK('${pkg}', this.checked)">
    ...
    if (pkgs.length > 100) {
        html += `<div class="list-item"><div class="item-desc"><small>Showing 100 of ${pkgs.length} apps</small></div></div>`;
    }
...
// Toggle app in list
async function toggleGK(pkg, add) {
    try {
        if (add) {
            await exec(`echo "${pkg}" >> "${GK_FILE}"`);
        } else {
            const raw = await exec(`cat "${GK_FILE}" 2>/dev/null`);
            const lines = raw.split('\n')
                .filter(l => l.trim() !== pkg && l.trim() !== '' && !l.startsWith('#'));
            await exec(`echo "${lines.join('\n')}" > "${GK_FILE}"`);
        }
    } catch (e) {
        console.error('GameKiller toggle failed:', e);
    }}

// DON'T auto-load on DOMContentLoaded - wait for user to expand

/* =========================================
   GLOBAL ROOT SEARCH (NAME + CONTENT)
   Optimized with BusyBox
   ========================================= */
let searchCache = {};
const CACHE_FILE = "/sdcard/MTK_AI_Engine/search_cache.json";
// ✅ Define BusyBox Path
const BB = "/data/adb/modules/MTK_AI/busybox";

// Load Cache on Start
async function loadSearchCache() {
    try {
        // Use BusyBox cat for consistency
        const data = await exec(`${BB} cat ${CACHE_FILE} 2>/dev/null`);
        if (data.trim()) {
...
async function startGlobalSearch() {
    ...
    const searchContent = document.getElementById('search-mode-content').checked;
    const searchName = document.getElementById('search-mode-name').checked;
    const statusDiv = document.getElementById('search-status');
    const resultsDiv = document.getElementById('search-results-container');

    if (!keyword) { alert("Enter a keyword!"); return; }
    if (!searchContent && !searchName) { alert("Select at least one mode!"); return; }

    const cacheKey = `${keyword}_${searchContent}_${searchName}`;

    // 1. Check Cache
    if (searchCache[cacheKey]) {
        statusDiv.style.display = 'block';
        statusDiv.textContent = "💾 Loaded from Cache (Files Only)";
    ...
    // 2. Start Safe Search
    statusDiv.style.display = 'block';
    statusDiv.textContent = "🔍 Scanning Safe Paths (Files Only)...";
    statusDiv.style.color = "#FF9F0A";
    resultsDiv.innerHTML = '<div style="padding:20px; text-align:center;">⏳ Filtering directories out...</div>';

    setTimeout(async () => {
        try {            // ⚠️ SAFE PATHS ONLY
            const safeBasePaths = "/sys/class /sys/devices /sys/ /dev/ /vendor/ /sys/module /proc/sys /proc/thread-self /proc/swpm /proc/ppm /proc/pmsr /proc/net /proc/mtktz /proc/mtk_lpm /proc/gpufreq /proc/gpufreqv2 /proc/fs /proc/driver /proc/device-tree /proc/cpufreq /proc/asound /data";
            ...
            if (searchName && !searchContent) {
                // MODE: Filename Only
                // ✅ Using BusyBox find
                const cmd = `${BB} find ${safeBasePaths} -type f -name "*${keyword}*" 2>/dev/null | ${BB} head -n 999`;
                ...
                // MODE: Content Only
                // ✅ Using BusyBox grep (faster & safer binary handling)
                const cmd = `${BB} grep -ril --binary-files=without-match "${keyword}" ${safeBasePaths} 2>/dev/null | ${BB} head -n 2000`;
                const output = await exec(cmd);
                results = output.split('\n').filter(l => l.trim() !== "");
            } 
            else {
                ...
                // Name Search (Files Only) - BusyBox find
                const nameOut = await exec(`${BB} find ${safeBasePaths} -type f -name "*${keyword}*" 2>/dev/null`);
                const nameRes = nameOut.split('\n').filter(l => l.trim() !== "");
                
                // Content Search - BusyBox grep
                const contentOut = await exec(`${BB} grep -ril --binary-files=without-match "${keyword}" ${safeBasePaths} 2>/dev/null`);
                const contentRes = contentOut.split('\n').filter(l => l.trim() !== "");

                // Combine & Deduplicate
                results = [...new Set([...nameRes, ...contentRes])].slice(0, 2000);
            }

            if (results.length === 0) {
                statusDiv.textContent = "❌ No files found.";
                statusDiv.style.color = "#ff453a";
                resultsDiv.innerHTML = '<div style="padding:20px; text-align:center; color:#666;">No matching <b>files</b> found.<br><small>(Directories were ignored)</small></div>';
                return;
            }
            ...
            statusDiv.textContent = `✅ Found ${results.length} files! (Cached)`;
            statusDiv.style.color = "#32D74B";
            renderSearchResults(results);
        } catch (err) {
            console.error("Search Error:", err);
            statusDiv.textContent = "⚠️ Search interrupted.";
            statusDiv.style.color = "#ff453a";
            resultsDiv.innerHTML = '<div style="color:#ff453a; text-align:center;">Search stopped.</div>';
        }
    }, 2000);
}

async function saveCacheToSD() {
    try {
        const json = JSON.stringify(searchCache);
        const safeJson = json.replace(/"/g, '\\"'); 
        // ✅ Using BusyBox echo for writing
        await exec(`${BB} echo "${safeJson}" > ${CACHE_FILE}`);
        document.getElementById('cache-count').textContent = Object.keys(searchCache).length;
...
function clearSearchCache() {
    searchCache = {};
    // ✅ Using BusyBox rm
    exec(`${BB} rm ${CACHE_FILE} 2>/dev/null`);
    document.getElementById('cache-count').textContent = "0";
    document.getElementById('search-results-container').innerHTML = '<div style="padding:20px; text-align:center; color:#666;">Cache cleared.</div>';
    ...
    paths.forEach(path => {
        ...
        <div class="list-item" style="padding: 12px; background: rgba(255,255,255,0.03); margin-bottom: 8px; border-radius: 8px; display:flex; justify-content:space-between; align-items:center; border: 1px solid rgba(255,255,255,0.05);">
            <div style="flex:1; min-width:0; margin-right: 10px;">
                <div style="font-weight:bold; color:#fff; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${fileName}</div>
                <div style="font-size:10px; color:#8E8E93; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${cleanPath}</div>
            </div>
            <button onclick="openFileEditor('${cleanPath}')" 
...
/* =========================================   UNIVERSAL FILE EDITOR (Handles ALL Types)
   Optimized with BusyBox
   ========================================= */
let currentEditPath = "";
let originalValue = ""; 
...
async function openFileEditor(path) {
    ...
    const pathDisplay = document.getElementById('editor-path-display');
    const currentPermDisplay = document.getElementById('editor-current-perm');
    const permInput = document.getElementById('editor-new-perm');
    const permStatus = document.getElementById('perm-status');
    const valueInput = document.getElementById('editor-value-input');
    const status = document.getElementById('editor-status');
    
    // Reset UI
    pathDisplay.textContent = path;
    currentPermDisplay.textContent = "...";
    permInput.value = "";
    permStatus.textContent = "";
    valueInput.value = "⏳ Reading raw data...";
    valueInput.style.color = "#8E8E93";
    status.textContent = "";
    modal.style.display = 'flex';

    try {
        // 1. Get File Info (Permissions & Size) - ✅ Using BusyBox stat
        const statInfo = await exec(`${BB} stat -c "%A (%a) | Size: %s bytes" "${path}" 2>/dev/null`);
        
        const permMatch = statInfo.match(/\((\d+)\)/);
        if (permMatch) {
            currentPermDisplay.textContent = permMatch[1];
            currentPermDisplay.style.color = "#FF9F0A";
        }

        // 2. Read Content Safely - ✅ Using BusyBox cat
        let content = "";
        try {
            content = await exec(`${BB} cat "${path}" 2>/dev/null`);
            if (content.includes('\u0000')) {
                status.innerHTML = "⚠️ <span style='color:#FF9F0A'>Binary file detected.</span>";
                // ✅ Using BusyBox xxd and fold
                const hexDump = await exec(`${BB} xxd -p "${path}" 2>/dev/null | ${BB} fold -w 60`);
                content = "--- HEX DUMP ---\n" + hexDump;
            }
        } catch (e) {
            throw new Error("Cannot read file directly");
        }
        originalValue = content;
        pathDisplay.innerHTML = `<span style="color:#8E8E93">${statInfo.trim()}</span><br><span style="color:#fff; word-break:break-all;">${path}</span>`;
        
        valueInput.value = content.trim();
        valueInput.style.color = "#32D74B";
        valueInput.style.height = "auto";
        valueInput.style.height = (valueInput.scrollHeight) + "px";

    } catch (err) {
        console.error("Read Error:", err);
        valueInput.value = "❌ Error: Could not read file.\n\nError: " + err.message;
        valueInput.style.color = "#ff453a";
        status.textContent = "⚠️ Unreadable File";
        status.style.color = "#ff453a";
    }
}

// 🆕 NEW: Change File Permissions Function
async function changeFilePermissions() {
    const newPerm = document.getElementById('editor-new-perm').value.trim();
    const statusLabel = document.getElementById('perm-status');
    const currentDisplay = document.getElementById('editor-current-perm');

    if (!newPerm) {
        statusLabel.textContent = "Enter value (e.g. 644)";
        statusLabel.style.color = "#FF9F0A";
        return;
    }

    try {
        // Execute chmod (System chmod is usually fine, but can use BB if needed)
        await exec(`chmod ${newPerm} "${currentEditPath}"`);
        
        statusLabel.textContent = `✅ Changed to ${newPerm}`;
        statusLabel.style.color = "#32D74B";
        
        // Update the displayed permission - ✅ Using BusyBox stat
        const updatedStat = await exec(`${BB} stat -c "%a" "${currentEditPath}" 2>/dev/null`);
        currentDisplay.textContent = updatedStat.trim();
        
        document.getElementById('editor-new-perm').value = "";
...
function closeEditor() {
    document.getElementById('file-editor-modal').style.display = 'none';    currentEditPath = "";
    originalValue = "";
}

async function saveFileValue() {
    const newValue = document.getElementById('editor-value-input').value;
    const status = document.getElementById('editor-status');
    ...
    try {
        if (newValue.startsWith("--- HEX DUMP")) {
            alert("⚠️ Hex Editing is advanced.\nThis script currently supports Text/Number editing only.\nPlease convert hex back to text manually or use a binary editor.");
            status.textContent = "❌ Hex Edit Not Supported Directly";
            status.style.color = "#ff453a";
            return;
        }

        // Standard Write - ✅ Using BusyBox echo and tee
        await exec(`${BB} echo "${newValue}" | ${BB} tee "${currentEditPath}" > /dev/null`);
        
        status.textContent = "✅ Saved Successfully!";
        status.style.color = "#32D74B";
        
        const btn = document.querySelector('button[onclick="saveFileValue()"]');
        ...
        if (err.message.includes("Permission denied")) {
            status.innerHTML += "<br><small>Try remounting RW: <code>mount -o rw,remount /sys</code></small>";
        }
    }
}
...
/* =========================================
   PERSISTENT VALUE SAVER (SD CARD)
   ========================================= */
const SAVE_DIR = "/sdcard/MTK_AI_Engine/saved_values";
const RESTORE_SCRIPT = "/data/adb/service.d/99_mtk_ai_restore_values.sh";

// Initialize Save Directory
async function initSaveDirectory() {
    try {
        await exec(`/data/adb/modules/MTK_AI/busybox mkdir -p ${SAVE_DIR} 2>/dev/null`);
    } catch (e) { console.warn("Could not create save dir", e); }
}

// Save Current Value to SD Card
async function saveValueToSD() {
    const path = currentEditPath;
    const value = document.getElementById('editor-value-input').value.trim();
    const status = document.getElementById('editor-status');
    ...
    try {
        ...
        // Create a safe filename from the path (replace / with _)
        // Example: /sys/class/power_supply/battery/input_current_limit -> _sys_class_power_supply_battery_input_current_limit
        const safeFilename = path.replace(/\//g, '_');
        const saveFile = `${SAVE_DIR}/${safeFilename}.conf`;

        // Write: PATH=VALUE
        await exec(`/data/adb/modules/MTK_AI/busybox echo "${path}=${value}" > "${saveFile}"`);

        status.innerHTML = `<span style="color:#32D74B">✅ Value Saved to SD! <br><small>Will restore on reboot.</small></span>`;
        
        // Optional: Auto-generate restore script
        generateRestoreScript();

    } catch (err) {
        status.innerHTML = `<span style="color:#ff453a">❌ Save Failed: ${err.message}</span>`;
    }
}

// Generate the Boot Script to Restore Values
// Generate the Boot Script to Restore Values
async function generateRestoreScript() {
    try {
        // Define the script content clearly
...
`;
        ...
        // ✅ FIX: Write using 'tee' instead of 'echo' to handle newlines safely
        // We pass the string directly to the exec function's stdin capability (if supported) 
        // OR simply write line-by-line or use a temporary file approach.
        
        // Most reliable method for WebUI exec: Write line by line or use printf with escaped newlines.
        // However, the cleanest trick is to base64 encode the content, then decode on device.
        
        const encoded = btoa(unescape(encodeURIComponent(scriptContent)));
        await exec(`echo "${encoded}" | /data/adb/modules/MTK_AI/busybox base64 -d > ${RESTORE_SCRIPT}`);
        
        await exec(`chmod 755 ${RESTORE_SCRIPT}`);
        
        console.log("✅ Restore script generated successfully.");
    } catch (e) {
        console.warn("Could not generate restore script", e);
        alert("Failed to create boot script: " + e.message);
    }
}
...
async function toggleMTKServices() {
    try {
        ...
        if (mtkServicesEnabled) {
            ...
            if (servicesStatus) {
                servicesStatus.textContent = "MANUAL MODE";
...
`);
            ...
            if (servicesStatus) {
                servicesStatus.textContent = "AUTO MODE";
...
// =================================================================
// GPU FREQUENCY CONTROL - FIXED SD CARD SAVE
// =================================================================
...
let oppCount = 33;
let currentOppIndex = 0;let isApplying = false;
let debugInfo = {};
...
async function detectGpuDriver() {
    ...
    try {
        const v2Data = await exec('cat /proc/gpufreqv2/gpu_working_opp_table 2>/dev/null');
        debugInfo.v2Exists = v2Data ? 'YES' : 'NO';
        
        if (v2Data && v2Data.includes('freq')) {
            gpuDriverType = 'gpufreqv2';
            debugInfo.detected = 'gpufreqv2';
            return 'gpufreqv2';
        }
    } catch (e) {
        debugInfo.v2Error = e.message;
    }
    
    try {
        const legacyData = await exec('cat /proc/gpufreq/gpufreq_opp_dump 2>/dev/null');
        debugInfo.legacyExists = legacyData ? 'YES' : 'NO';
        
        if (legacyData && legacyData.includes('freq')) {
            gpuDriverType = 'gpufreq';
            debugInfo.detected = 'gpufreq';
            return 'gpufreq';
        }
    } catch (e) {
        debugInfo.legacyError = e.message;
    }
...
async function buildGpuFrequencyMap() {
    ...
    try {
        ...
        if (gpuDriverType === 'gpufreqv2') {
            data = await exec('cat /proc/gpufreqv2/gpu_working_opp_table 2>/dev/null');
        } else if (gpuDriverType === 'gpufreq') {
            data = await exec('cat /proc/gpufreq/gpufreq_opp_dump 2>/dev/null');        }
        
        if (data) {
            const lines = data.trim().split('\n');
            for (const line of lines) {
                // ✅ Matches both "freq:" and "freq ="
    ...
    const freqs = Object.values(gpuFrequencyMap);
    detectedMinFreq = Math.min(...freqs);
    detectedMaxFreq = Math.max(...freqs);
...
function updateGpuDisplay(oppIndex) {
    ...
    const freq = gpuFrequencyMap[oppIndex];
    if (freq !== undefined) {
        display.textContent = `${freq} MHz`;
        display.style.color = 'var(--color-blue)';
    } else {
        display.textContent = `${detectedMaxFreq} MHz`;
        display.style.color = '#FF9F0A';
    }
}
// 🔹 Update Labels
function updateFrequencyLabels() {
    const labels = document.getElementById('gpu-frequency-labels');
    if (labels) {
        labels.innerHTML = `<span>${detectedMinFreq} MHz</span><span>${detectedMaxFreq} MHz</span>`;
    }
}

// 🔹 Update Driver Info
function updateDriverInfo() {
    const info = document.getElementById('gpu-driver-info');
    if (info) {
        if (gpuDriverType === 'gpufreqv2') {
            info.textContent = '📱 MediaTek gpufreqv2 (OPP Index)';
            info.style.color = '#32D74B';
        } else if (gpuDriverType === 'gpufreq') {
            info.textContent = '📱 MediaTek gpufreq (Frequency KHz)';
            info.style.color = '#32D74B';
        } else {
            info.textContent = '⚠️ Unknown GPU - Using Fallback';
            info.style.color = '#FF9F0A';
        }
    }
}
...
function showDebugInfo() {
    const debugDiv = document.getElementById('gpu-debug-info');
    const debugText = document.getElementById('gpu-debug-text');
...
// 🔹 Status Message
function showGpuStatus(msg, type = 'info') {
    const status = document.getElementById('gpu-status-msg');
    if (!status) return;
    
    const colors = {
        'success': { bg: 'rgba(50,215,75,0.15)', color: '#32D74B', border: 'rgba(50,215,75,0.4)' },
        'error': { bg: 'rgba(255,69,58,0.15)', color: '#FF453A', border: 'rgba(255,69,58,0.4)' },
        'warning': { bg: 'rgba(255,159,10,0.15)', color: '#FF9F0A', border: 'rgba(255,159,10,0.4)' },
        'info': { bg: 'rgba(0,122,255,0.15)', color: '#007AFF', border: 'rgba(0,122,255,0.4)' }
    };
    
    const scheme = colors[type] || colors.info;
    status.style.background = scheme.bg;
    status.style.color = scheme.color;    status.style.border = `1px solid ${scheme.border}`;
    status.textContent = msg;
}

// 🔹 Create Config Directory (FIXED)
async function ensureConfigDir() {
    try {
        // Method 1: Try mkdir directly
        await exec(`mkdir -p ${CONFIG_DIR}`);
        
        // Method 2: Verify it exists
        const check = await exec(`ls -d ${CONFIG_DIR} 2>/dev/null`);
        if (check && check.includes('MTK_AI_Engine')) {
            debugInfo.dirCreated = 'YES';
            return true;
        }
    } catch (e) {
    ...
    // Method 3: Try with su (if needed)
    try {
        await exec(`su -c "mkdir -p ${CONFIG_DIR}"`);
        const check = await exec(`ls -d ${CONFIG_DIR} 2>/dev/null`);
        if (check) {
            debugInfo.dirCreated = 'YES (via su)';
            return true;
        }
    } catch (e) {
        debugInfo.dirSuError = e.message;
    }
    
    debugInfo.dirCreated = 'NO';
    return false;
}

// 🔹 Save to SD Card (FIXED - Multiple Methods)
async function saveToSdCard(oppIndex) {
    debugInfo.saveAttempts = [];
    
    // Method 1: Direct write
    try {
        await exec(`echo "${oppIndex}" > ${CONFIG_FILE}`);
        const verify = await exec(`cat ${CONFIG_FILE} 2>/dev/null`);
        if (verify?.trim() === oppIndex.toString()) {
            debugInfo.saveMethod = 'Direct write';
            debugInfo.saveSuccess = 'YES';
            return true;
        }
        debugInfo.saveAttempts.push('Direct: verify failed');    } catch (e) {
        debugInfo.saveAttempts.push(`Direct: ${e.message}`);
    }
    
    // Method 2: With su
    try {
        await exec(`su -c "echo '${oppIndex}' > ${CONFIG_FILE}"`);
        const verify = await exec(`cat ${CONFIG_FILE} 2>/dev/null`);
        if (verify?.trim() === oppIndex.toString()) {
        ...
        debugInfo.saveAttempts.push('su: verify failed');
    } catch (e) {
        debugInfo.saveAttempts.push(`su: ${e.message}`);
    }
    
    // Method 3: Using tee
    try {
        await exec(`echo "${oppIndex}" | su -c "tee ${CONFIG_FILE} > /dev/null"`);
        const verify = await exec(`cat ${CONFIG_FILE} 2>/dev/null`);
        if (verify?.trim() === oppIndex.toString()) {
            debugInfo.saveMethod = 'tee';
            debugInfo.saveSuccess = 'YES';
            return true;
        }
        debugInfo.saveAttempts.push('tee: verify failed');
    } catch (e) {
        debugInfo.saveAttempts.push(`tee: ${e.message}`);
    }
    
    // Method 4: Using printf
    try {
        await exec(`su -c "printf '${oppIndex}' > ${CONFIG_FILE}"`);
        const verify = await exec(`cat ${CONFIG_FILE} 2>/dev/null`);
        if (verify?.trim() === oppIndex.toString()) {
            debugInfo.saveMethod = 'printf';
            debugInfo.saveSuccess = 'YES';
            return true;
        }
        debugInfo.saveAttempts.push('printf: verify failed');
    } catch (e) {
        debugInfo.saveAttempts.push(`printf: ${e.message}`);
    }
...
async function loadFromSdCard() {
    try {
        const saved = await exec(`cat ${CONFIG_FILE} 2>/dev/null`);
        if (saved?.trim()) {
            const idx = parseInt(saved.trim());
            if (!isNaN(idx)) {
                debugInfo.loadedFromSd = 'YES';
                debugInfo.savedValue = idx;
                return idx;
            }
        }
        debugInfo.loadedFromSd = 'NO (file empty)';
    } catch (e) {
...
async function loadGpuSettings() {
    showGpuStatus('🔍 Detecting GPU...', 'info');
    
    try {
        await detectGpuDriver();
        await buildGpuFrequencyMap();
        
        // Load saved index (try SD card first, then localStorage)
        let savedIndex = await loadFromSdCard();
        
        if (savedIndex === null) {
            const local = localStorage.getItem('gpu_opp_index');
            if (local) {
                savedIndex = parseInt(local);
                debugInfo.loadedFrom = 'localStorage';
            }
        } else {
            debugInfo.loadedFrom = 'SD card';
        }
        
        currentOppIndex = Math.max(0, Math.min(savedIndex || 0, oppCount - 1));
        ...
        if (slider) {
            ...
            updateGpuDisplay(currentOppIndex);
            updateFrequencyLabels();
            updateDriverInfo();        }
        
        showGpuStatus(`✅ ${gpuDriverType || 'Unknown'} | ${oppCount} OPPs | ${detectedMinFreq}-${detectedMaxFreq} MHz`, 'success');
        showDebugInfo();
        console.log('GPU Debug:', debugInfo);
    } catch (error) {
        console.error('GPU load error:', error);
        showGpuStatus('⚠️ Detection failed - Using defaults', 'warning');
    }
}

// 🔹 Slider Input Handler
function handleGpuSliderInput(event) {
    const value = parseInt(event.target.value);
    currentOppIndex = (oppCount - 1) - value;
    updateGpuDisplay(currentOppIndex);
    showGpuStatus(`👆 Selected: ${gpuFrequencyMap[currentOppIndex] || '?'} MHz`, 'info');
}

// 🔹 Apply Settings (FIXED SAVE)
async function applyGlobalGpuSettings() {
    if (isApplying) {
        showGpuStatus('⏳ Already applying...', 'warning');
        return;
    }
    isApplying = true;
    
    const oppIndex = currentOppIndex;
    const freqMhz = gpuFrequencyMap[oppIndex] || detectedMaxFreq;
    const freqKhz = freqMhz * 1000;
    
    showGpuStatus(`⏳ Applying ${freqMhz} MHz...`, 'warning');
    ...
    try {
        ...
        // ⭐ STEP 2: Save to SD card (with verification)
        const saveSuccess = await saveToSdCard(oppIndex);
        
        // ⭐ STEP 3: Also save to localStorage (backup)
        localStorage.setItem('gpu_opp_index', oppIndex.toString());
        debugInfo.localStorageSave = 'YES';
                // ⭐ STEP 4: Apply GPU setting
        if (gpuDriverType === 'gpufreqv2') {
            await exec(`su -c "echo ${oppIndex} > /proc/gpufreqv2/fix_target_opp_index"`);
            debugInfo.execCmd = `echo ${oppIndex} > /proc/gpufreqv2/fix_target_opp_index`;
        } else if (gpuDriverType === 'gpufreq') {
            await exec(`su -c "echo ${freqKhz} > /proc/gpufreq/gpufreq_opp_freq"`);
            await exec(`su -c "echo ${oppIndex} > /proc/gpufreq/gpufreq_sb_idx" 2>/dev/null`);
            debugInfo.execCmd = `echo ${freqKhz} > /proc/gpufreq/gpufreq_opp_freq`;
        } else {
            await exec(`su -c "echo ${oppIndex} > /proc/gpufreqv2/fix_target_opp_index" 2>/dev/null`);
            await exec(`su -c "echo ${freqKhz} > /proc/gpufreq/gpufreq_opp_freq" 2>/dev/null`);
        }
        
        // ⭐ STEP 5: Verify the change
        try {
            if (gpuDriverType === 'gpufreq') {
                const verify = await exec('cat /proc/gpufreq/gpufreq_opp_freq 2>/dev/null');
                debugInfo.verifyFreq = verify?.trim();
            } else if (gpuDriverType === 'gpufreqv2') {
                const verify = await exec('cat /proc/gpufreqv2/fix_target_opp_index 2>/dev/null');
                debugInfo.verifyIdx = verify?.trim();
            }
        } catch (e) {
            debugInfo.verifyError = e.message;
        }
        
        // ⭐ STEP 6: Verify SD card save
        try {
            const sdVerify = await exec(`cat ${CONFIG_FILE} 2>/dev/null`);
            debugInfo.sdVerify = sdVerify?.trim();
        } catch (e) {
            debugInfo.sdVerifyError = e.message;
        }
        
        // Show result
        if (saveSuccess) {
            showGpuStatus(`✅ SUCCESS: ${freqMhz} MHz | Saved to SD`, 'success');
        } else {
            showGpuStatus(`⚠️ Applied but save failed (see debug)`, 'warning');
        }
        ...
        debugInfo.execError = error.message;
        showGpuStatus(`❌ FAILED: ${error.message}`, 'error');
        showDebugInfo();
    }
        if (applyBtn) {
...
let allAppsData = []; // ✅ Store full app list for filtering
...
async function loadAppList() {
    ...
    try {
        ...
        const raw = await exec("pm list packages | cut -d: -f2 | sort");
        ...
        // ✅ STORE APP DATA FOR FILTERING
        allAppsData = sortedPkgs.map(pkg => ({
            pkg: pkg,
            shortName: pkg.split('.').pop().toUpperCase(),
            isAdded: addedList.includes(pkg)
        }));
...
async function addGame(pkg) {
    ...
    const searchQuery = document.getElementById('app-search')?.value || '';
    await loadAppList(); // Reloads and clears search
    if (searchQuery) {
        document.getElementById('app-search').value = searchQuery;
        filterAppList(searchQuery); // Re-apply filter
    }
    syncLists();
}
...
async function removeGame(pkg) {
    ...
    if (searchQuery) {
        document.getElementById('app-search').value = searchQuery;
        filterAppList(searchQuery);
    }
    syncLists();
}

// ✅ Renders app list from provided data array
function renderAppList(apps) {
    const container = document.getElementById('app-list-container');
    if (!container) return;
    
    if (apps.length === 0) {
        container.innerHTML = '<div style="text-align:center; padding:20px; color:#888;">No apps match your search</div>';
        return;
    }
    
    let html = '';
    apps.forEach(app => {
        const { pkg, shortName, isAdded } = app;
...
function filterAppList(query) {
    ...
    const filtered = allAppsData.filter(app => 
        app.pkg.toLowerCase().includes(searchTerm) || 
        app.shortName.toLowerCase().includes(searchTerm)
    );
...
async function openAppPopup(pkg) {
    // ✅ CRITICAL: Clean up ALL overlays first
    document.querySelectorAll('.popup-overlay').forEach(overlay => {
        if (overlay.id !== 'floating-popup') {
    ...
    // Also hide other popups
    document.getElementById('cpu-scaling-popup')?.style.setProperty('display', 'none');
    document.getElementById('monitor-popup')?.style.setProperty('display', 'none');
    
    // Remove body class that might block clicks
    document.body.classList.remove('popup-open');
    
    // NOW open floating-popup
    currentTargetPkg = pkg;
    currentMonitorPkg = pkg;  // ← Critical: Set monitor package!
    
    document.getElementById('popup-pkg-display').innerText = pkg;
    
    // Update monitor badge
    const monitorBadge = document.getElementById('monitor-pkg-display');
    if (monitorBadge) monitorBadge.textContent = pkg;
    ...
    // === ✅ MONITORING: Reset & Initialize (SEPARATE from Promise.all) ===
    resetMonitorUI();           // Clear old stats (synchronous)
    initMonitorToggle();        // Init toggle button state (async, but not data-loading)

    // Set defaults for other controls
    ...
    // Reset CPU display
    const cpuDisplay = document.getElementById('cpu-max-display');
    if (cpuDisplay) {
        cpuDisplay.textContent = "Detecting...";
        cpuDisplay.style.color = "#86868b";
    }
    ...
    // === ✅ LOAD DATA IN PARALLEL (ONLY data-loading functions) ===
    Promise.all([
        loadEemData(pkg),
        loadScalingData(pkg),
        ...
        loadVsyncData(pkg),
        loadRefreshData(pkg),
        loadCPUPercentData(pkg), 
        loadAppResolutionConfig(pkg), 
        loadCustomCommand(pkg)
        // ❌ REMOVE: initMonitorToggle(pkg) — not a data loader!
    ]).catch(err => console.warn("Some config failed to load:", err));
    
    // === ✅ OPTIONAL: Load saved section state ===
    setTimeout(() => {
    }, 100);
}
...
// Detect stock CPU max (cached)
async function detectCPUStockMax() {
    if (cpuStockMax > 0) return cpuStockMax;
    try {
        const raw = await exec('cat /sys/devices/system/cpu/cpufreq/policy0/cpuinfo_max_freq 2>/dev/null');
        cpuStockMax = parseInt(raw.trim()) || 3000000;
    } catch (e) {
        cpuStockMax = 3000000;
    }
    return cpuStockMax;
...
// Load CPU percent for app
async function loadCPUPercentData(pkg) {
    const stockMax = await detectCPUStockMax();
    
    let savedPercent = null;
    try {
        const raw = await exec(`cat /sdcard/MTK_AI_Engine/per_app/${pkg}.cpu_percent 2>/dev/null`);
        savedPercent = parseInt(raw.trim()) || null;
    } catch (e) {}
    
    const displayPercent = savedPercent !== null ? savedPercent : 100;
    
    const slider = document.getElementById('cpu-percent-slider');
    const display = document.getElementById('cpu-percent-display');
    
    if (slider) {
        slider.value = displayPercent;
        slider.oninput = debounce(() => autoApplyCPUPercent(pkg), 250);
    }
    
    if (display) {
        display.textContent = `${displayPercent}%`;
        display.style.color = (savedPercent !== null && savedPercent !== 100) ? '#32D74B' : '#86868b';
    }
}

// Auto-apply CPU percent (YOUR SHELL LOGIC)
async function autoApplyCPUPercent(pkg) {
    if (!pkg) return;
    
    const slider = document.getElementById('cpu-percent-slider');
    const display = document.getElementById('cpu-percent-display');
    const status = document.getElementById('cpu-percent-status');
    ...
    if (status) {
        status.textContent = `💾 Saving ${percent}%...`;
        status.style.color = "#0A84FF";
    }
    
    try {
        // ✅ YOUR EXACT SHELL LOGIC - Batched into ONE command
        const shellCmd = `
            for cpu in /sys/devices/system/cpu/cpu[0-9]*; do
                if [ -f "\$cpu/cpufreq/cpuinfo_max_freq" ]; then
                    MAX_FREQ=\$(cat "\$cpu/cpufreq/cpuinfo_max_freq")
                    MIN_FREQ=\$(cat "\$cpu/cpufreq/cpuinfo_min_freq")
                    TARGET_MAX=\$((MAX_FREQ * ${percent} / 100))
                    echo \$MIN_FREQ > "\$cpu/cpufreq/scaling_min_freq"
                    echo \$TARGET_MAX > "\$cpu/cpufreq/scaling_max_freq"
                fi
            done &&
            mkdir -p /sdcard/MTK_AI_Engine/per_app &&
            echo '${percent}' > /sdcard/MTK_AI_Engine/per_app/${pkg}.cpu_percent
        `;
        
        await exec(`su -c "${shellCmd}"`);
        
        // Success feedback
        if (status) {
            status.textContent = `✅ ${percent}% applied`;
            status.style.color = "#32D74B";
            setTimeout(() => { if (status.textContent.includes('applied')) status.textContent = ''; }, 2000);
        }
        if (display) {
            display.style.color = (percent === 100) ? '#86868b' : '#32D74B';
        }
        
    } catch (e) {
        console.error("CPU apply failed:", e);
        if (status) {
            status.textContent = `❌ Failed`;
            status.style.color = "#FF453A";
            setTimeout(() => { if (status.textContent === '❌ Failed') status.textContent = ''; }, 2000);
        }
    }
}
// Reset to stock
async function resetCPUPercent() {
    const status = document.getElementById('cpu-percent-status');
    if (status) {
        status.textContent = "↺ Resetting...";
        status.style.color = "#636366";
    }
    ...
    try {
        ...
        const pkg = currentTargetPkg;
        if (pkg) {
            await exec(`rm -f /sdcard/MTK_AI_Engine/per_app/${pkg}.cpu_percent 2>/dev/null`);
            await loadCPUPercentData(pkg);
        }
        
        if (status) {
            status.textContent = "↺ Reset to stock";
            setTimeout(() => { if (status.textContent === '↺ Reset to stock') status.textContent = ''; }, 2000);
        }
        ...
        if (status) {
            ...
            status.style.color = "#FF453A";
            setTimeout(() => { if (status.textContent === '❌ Reset failed') status.textContent = ''; }, 2000);
        }
    }
}

/* =========================================
   CUSTOM SHELL COMMAND HELPERS
   ========================================= */
const CMD_CONFIG_DIR = "/sdcard/MTK_AI_Engine/per_app";
let pendingCmdSave = null;

// 1. Save Command (Debounced)
// 1. Save Command (Fixed with Base64)
function saveCustomCommand() {
    const input = document.getElementById('custom-cmd-input');
    const icon = document.getElementById('cmd-status-icon');
    
    if (!input || !currentTargetPkg) return;
    ...
    // Debounce
    if (pendingCmdSave) clearTimeout(pendingCmdSave);
    
    pendingCmdSave = setTimeout(async () => {
        try {
            if (value) {
                // ✅ FIX: Encode to Base64 to avoid all quote/escape issues
                // btoa handles ASCII/UTF-8 strings safely
                const encodedCmd = btoa(unescape(encodeURIComponent(value)));
                
                // Write the encoded string to file
                const cmd = `su -c "mkdir -p ${CMD_CONFIG_DIR} && echo '${encodedCmd}' > '${CMD_CONFIG_DIR}/${currentTargetPkg}.cmd'"`;
                await exec(cmd);
                
                if(icon) icon.textContent = "✅";
            } else {
                // Delete file if empty
                await exec(`rm -f '${CMD_CONFIG_DIR}/${currentTargetPkg}.cmd'`);
                if(icon) icon.textContent = "🗑️";
            }
            setTimeout(() => { if(icon) icon.textContent = "💾"; }, 1500);
        } catch (e) {
            console.error("Cmd save failed", e);
            if(icon) icon.textContent = "❌";
...
// 2. Load Command on Popup Open
async function loadCustomCommand(pkg) {
    const configFile = `${CMD_CONFIG_DIR}/${pkg}.cmd`;
    const input = document.getElementById('custom-cmd-input');
    
    if (!input) return;

    try {
        const raw = await exec(`cat '${configFile}' 2>/dev/null`);
        // Handle potential newlines in the saved file
...
async function testCustomCommand() {
    ...
    try {
        // Execute immediately with su
        await exec(`su -c "${cmd.replace(/"/g, '\\"')}"`);
        alert("✅ Command executed successfully!\nCheck logs for output.");
        if(statusIcon) statusIcon.textContent = "✅";
        setTimeout(() => { if(statusIcon) statusIcon.textContent = "💾"; }, 2000);
    } catch (e) {
...
function clearCustomCommand() {
    ...
    saveCustomCommand(); // Trigger save (which will delete the file)
}

/* =========================================
   LIGHTWEIGHT RESOLUTION CONFIG LOADER
   ========================================= */
const RES_CONFIG_DIR = "/sdcard/MTK_AI_Engine/per_app";
let pendingResolutionSave = null; // Debounce holder

// 1. Handle Dropdown Change
function handleResolutionSelect() {
    const select = document.getElementById('resolution-select');
    const customInput = document.getElementById('resolution-custom-input');
    
    if (select.value === 'custom') {
        customInput.style.display = 'block';
        customInput.focus();
        pendingResolutionSave = null; // Wait for typing
    } else {
        customInput.style.display = 'none';
        saveResolutionConfig(currentTargetPkg, select.value);
    }
}

// 2. Save Config (Debounced & Simple)
function saveCustomResolution() {
    ...
    if (select.value === 'custom' && customInput.value.trim()) {
        ...
        if (pendingResolutionSave) clearTimeout(pendingResolutionSave);
        
        // Wait 500ms after typing stops
        pendingResolutionSave = setTimeout(() => {
            saveResolutionConfig(currentTargetPkg, customInput.value.trim());
        }, 500);
    }
}

async function saveResolutionConfig(pkg, value) {
    if (!pkg || !value) return;
    ...
    try {
        // Single command: mkdir + write
        await exec(`su -c "mkdir -p ${RES_CONFIG_DIR} && echo '${value}' > '${RES_CONFIG_DIR}/${pkg}.resolution'"`);
        
        if(statusIcon) {
            statusIcon.textContent = "✅";
            setTimeout(() => statusIcon.textContent = "💾", 1500);
        }
    } catch (e) {
        console.error("Res save failed", e);
        if(statusIcon) statusIcon.textContent = "❌";
    }
}

// 3. Load Config (Non-Blocking)
async function loadAppResolutionConfig(pkg) {
    const configFile = `${RES_CONFIG_DIR}/${pkg}.resolution`;
    const select = document.getElementById('resolution-select');
    const customInput = document.getElementById('resolution-custom-input');
    ...
    try {
        // Simple cat command
        const raw = await exec(`cat '${configFile}' 2>/dev/null`);
        const savedValue = raw.trim();
        
        if (!savedValue) throw new Error("No config");

        const presets = ["native", "720x1280", "720x1600", "1080x1920", "1080x2400"];
        
        if (presets.includes(savedValue)) {
...
function toggleMonitor() {
    ...
    if (!currentMonitorPkg) {
        if (statusEl) statusEl.innerText = "❌ No app selected";
    ...
    if (isMonitorRunning) {
        ...
        setTimeout(() => {
            readStatsFile(false);
            if (statusEl) {
                statusEl.innerText = "✅ Monitor Stopped";
                statusEl.style.color = "#ff453a";
            }
            if (monitorInterval) { 
                clearInterval(monitorInterval); 
...
// === READ STATS ===
async function readStatsFile(showMsg) {    const filePath = `/sdcard/MTK_AI_Engine/stats_${currentMonitorPkg}.txt`;
    const statusEl = document.getElementById('monitor-status');
    ...
    try {
        ...
        if (showMsg && statusEl) {
            ...
            statusEl.style.color = "#4cd964";
            setTimeout(() => {
                if (statusEl.innerText === "✅ Updated") statusEl.innerText = "";
            }, 2000);
        }
    } catch (e) {
        if (showMsg && statusEl) {
...
function closeAllOtherPopups() {
    // Close CPU scaling popup
    const cpuPopup = document.getElementById('cpu-scaling-popup');
    if (cpuPopup) {
        cpuPopup.style.display = 'none';
        cpuPopup.classList.remove('active');
    }
    
    // Hide any other popup overlays
    document.querySelectorAll('.popup-overlay').forEach(popup => {
        if (popup.id !== 'floating-popup') {
...
function toggleSection(header) {
    ...
    document.querySelectorAll('.collapse-section').forEach(s => {
        if (s !== section) {
    ...
    // Save expanded state to localStorage (optional)
    saveSectionState();
}

// Expand/Collapse all sections
function toggleAllSections() {
    const allExpanded = document.querySelectorAll('.collapse-section.expanded').length === 
                        document.querySelectorAll('.collapse-section').length;
    ...
    // Update button text
    const btn = document.querySelector('.popup-title button');
    if (btn) {
        btn.textContent = allExpanded ? 'Expand All' : 'Collapse All';
    }
}

// Save expanded state (optional - persists across popup opens)
function saveSectionState() {
    const state = [];
    document.querySelectorAll('.collapse-section').forEach((section, index) => {
        if (section.classList.contains('expanded')) {            state.push(index);
        }
    });
    localStorage.setItem('popupSectionState', JSON.stringify(state));
}

// Load saved state (optional)
function loadSectionState() {
    try {
        const state = JSON.parse(localStorage.getItem('popupSectionState') || '[]');
        state.forEach(index => {
            const section = document.querySelectorAll('.collapse-section')[index];
