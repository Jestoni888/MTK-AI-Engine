...
// --- DYNAMIC zRAM & SWAPPINESS CONTROL ---
...
const refreshZramUI = async () => {
    ...
    let totalPhysicalMB = Math.floor(parseInt(memInfo) / 1024);
    if (!totalPhysicalMB || isNaN(totalPhysicalMB)) totalPhysicalMB = 4096;

    // Force Slider Max based on your 12GB RAM
    zSizeS.max = totalPhysicalMB; 
    zSizeS.step = 128;
    ...
    if (diskSizeRaw && diskSizeRaw !== "0") {
        const currentMB = Math.floor(parseInt(diskSizeRaw) / 1024 / 1024);
        const totalGB = (parseInt(diskSizeRaw) / 1024 / 1024 / 1024).toFixed(2);
        
        // This text is now protected from being overwritten by the monitor
        zStats.innerText = `ACTIVE | ${totalGB} GB zRAM / ${Math.round(totalPhysicalMB/1024)} GB Physical`;
        
        zSizeS.value = currentMB;
        zSizeT.innerText = currentMB + " MB";
    }
};

// Start UI values first
await refreshZramUI();

// FIX: This listener now has the correct 'max' context
zSizeS.oninput = function() {
    zSizeT.innerText = this.value + " MB";
};

zApply.onclick = async () => {
    const targetSize = zSizeS.value + "M"; 
    ...
    await exec(`
        ...
        # We store these in a file that the boot script will read later
        mkdir -p /data/adb/zram_config
        echo "SIZE=${targetSize}" > /data/adb/zram_config/settings.conf
        echo "ALGO=${targetAlgo}" >> /data/adb/zram_config/settings.conf
        echo "SWAP=${targetSwp}" >> /data/adb/zram_config/settings.conf
    `);

    zApply.style.opacity = "1";
    status.innerText = "Applied & Saved for Reboot!";
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            div.innerHTML = `
                ...
                <select class="cpuset-select" data-pkg="${pkg}" style="font-size:12px; padding:4px 8px;">
                    ...
                    <option value="background" ${current === 'background' ? 'selected' : ''}>Background (0-3 CPU)</option>
