...
// =================================================================
// UNIVERSAL GPU FREQUENCY CONTROL (MediaTek Dynamic)
// Supports: /proc/gpufreqv2/ AND /proc/gpufreq/ paths
// Dynamic OPP detection + fallback to safe defaults
// =================================================================

let gpuFrequencyMap = null;
let detectedMinFreq = 0;
let detectedMaxFreq = 0;
let detectedOppCount = 0;
let gpuControlPath = null; // Will store detected path

// 🔹 STEP 0: Detect GPU Control Path
async function detectGpuControlPath() {
    const paths = [
        '/proc/gpufreqv2/gpu_working_opp_table',  // Newer MediaTek
        '/proc/gpufreq/gpufreq_opp_dump',          // Older MediaTek (your device)
        '/proc/gpufreq/gpufreq_opp_freq',          // Fallback control file
        '/sys/devices/platform/soc/*/mali/devfreq/*/available_frequencies' // Generic
    ];
    ...
    for (const path of paths) {
        try {
            ...
            if (path.includes('*')) {
                const result = await exec(`ls ${path} 2>/dev/null | head -1`);
                if (result.trim()) {
                    gpuControlPath = result.trim();
                    console.log(`✅ Detected GPU path: ${gpuControlPath}`);
                    return true;
                }
            } else {
                await exec(`cat ${path} 2>/dev/null`);
                gpuControlPath = path;
                console.log(`✅ Detected GPU path: ${gpuControlPath}`);
                return true;
            }
    ...
    console.warn('⚠️ No standard GPU path detected, using fallback');
    gpuControlPath = 'fallback';
    return false;
}

// 🔹 STEP 1: Load Settings (Dynamic + Your Logic)
async function loadGpuSettings() {
    try {        // Detect path first
        await detectGpuControlPath();
        
        // Build frequency map based on detected path
        ...
        try {
            ...
            if (savedContent.trim() !== '') {
                ...
                // Validate against detected OPP count
                if (oppIndex >= detectedOppCount) oppIndex = 0;
                source = 'sdcard';
            }
        } catch (e) {
        ...
        if (source === 'kernel') {
            try {
                // Try different control files based on path
                let kernelOpp = '';
                if (gpuControlPath?.includes('gpufreqv2')) {
                    kernelOpp = await exec(`cat /proc/gpufreqv2/fix_target_opp_index 2>/dev/null`);
                } else if (gpuControlPath?.includes('gpufreq_opp_freq')) {
                    // For gpufreq, read current freq and map to OPP index
                    const currentFreq = await exec(`cat /proc/gpufreq/gpufreq_opp_freq 2>/dev/null`);
                    const freqKhz = parseInt(currentFreq.trim()) || 0;
                    // Find closest OPP index
                    for (const [idx, freqMhz] of Object.entries(gpuFrequencyMap)) {
                        if (freqMhz * 1000 === freqKhz) {
                            oppIndex = parseInt(idx);
                            break;
                        }
                    }
                }
                ...
                if (kernelOpp.trim()) {
                    oppIndex = parseInt(kernelOpp.trim());
                    if (oppIndex >= detectedOppCount) oppIndex = 0;
                }
            } catch (e) {
                oppIndex = 0; 
        ...
        // Update UI with DYNAMIC slider range
        const sliderPosition = (detectedOppCount - 1) - oppIndex;
        const slider = document.getElementById('gpu-opp-slider');
        if (slider) {
            slider.max = detectedOppCount - 1;  // Dynamic max
            slider.min = 0;
            slider.value = Math.max(0, Math.min(detectedOppCount - 1, sliderPosition));
            updateGpuDisplay(oppIndex);
            updateFrequencyLabels();
        }

        console.log(`GPU settings loaded from ${source}: OPP ${oppIndex}/${detectedOppCount-1} = ${gpuFrequencyMap?.[oppIndex] || 'N/A'} MHz`);
    } catch (error) {
...
// 🔹 STEP 2: Build Frequency Map (Universal Parser)
async function buildGpuFrequencyMap() {
    gpuFrequencyMap = {};
    detectedOppCount = 0;

    // --- PATH 1: gpufreqv2 format (your original code) ---
    if (gpuControlPath?.includes('gpufreqv2')) {
        try {
            const rawStatus = await exec(`cat /proc/gpufreqv2/gpu_working_opp_table 2>/dev/null`);
            ...
            if (rawStatus && rawStatus.includes('freq:')) {
                ...
                for (const line of lines) {
                    ...
                    if (match) {
                        ...
                        detectedOppCount = Math.max(detectedOppCount, idx + 1);
                    }
                }
            }        } catch (e) {
            console.warn("Error reading gpufreqv2:", e.message);
        }
    }
    
    // --- PATH 2: gpufreq format (YOUR DEVICE) ---
    if (gpuControlPath?.includes('gpufreq_opp_dump') || Object.keys(gpuFrequencyMap).length === 0) {
        try {
            const rawStatus = await exec(`cat /proc/gpufreq/gpufreq_opp_dump 2>/dev/null`);
            
            if (rawStatus && rawStatus.includes('freq:')) {
                const lines = rawStatus.trim().split('\n');
                
                for (const line of lines) {
                    // Match YOUR format: [00] freq: 950000 kHz, volt: 725000 uV, power: 1133
                    const match = line.match(/\[(\d+)\]\s*freq:\s*(\d+)\s*kHz/);
                ...
                if (detectedOppCount > 0) {
                    console.log(`✅ Detected ${detectedOppCount} OPPs from gpufreq_opp_dump`);
                }
            }
        } catch (e) {
            console.warn("Error reading gpufreq_opp_dump:", e.message);
        }
    }

    // --- FALLBACK: Dynamic 0-N OPP Index ---
    if (Object.keys(gpuFrequencyMap).length === 0) {
        console.warn("⚠️ GPU OPP table not detected. Using Dynamic Fallback.");
        
        // Safe generic range for MediaTek GPUs
        const defaultMin = 300;
        const defaultMax = 950;  // Your device's max
        const defaultOppCount = 11;  // Common for MediaTek (0-10)
        
        for (let i = 0; i < defaultOppCount; i++) {
            // Linear interpolation
            const freq = Math.round(defaultMax - ((defaultMax - defaultMin) * i / (defaultOppCount - 1)));
            gpuFrequencyMap[i] = freq;
        }        detectedOppCount = defaultOppCount;
    }
    ...
    console.log(`📊 GPU Range: ${detectedMinFreq}-${detectedMaxFreq} MHz | OPPs: 0-${detectedOppCount-1}`);
    return true;
}

// 🔹 STEP 3: Update Labels (Dynamic)
function updateFrequencyLabels() {
    const labelsDiv = document.getElementById('gpu-frequency-labels');
    if (!labelsDiv) return;
    
    labelsDiv.innerHTML = `
        <span class="gpu-label-min">${detectedMinFreq} MHz</span>
        <span class="gpu-label-max">${detectedMaxFreq} MHz</span>
    `;
}

// 🔹 STEP 4: Show Frequency (Dynamic + Fallback)
...
async function getGpuFrequencyForOpp(oppIndex) {
    try {
        ...
        // Try gpufreq (YOUR DEVICE)
        const status = await exec(`cat /proc/gpufreq/gpufreq_opp_dump 2>/dev/null`);
        if (status) {
            const lines = status.trim().split('\n');
            for (const line of lines) {
                // Match: [05] freq: 906000 kHz
                if (line.includes(`[${oppIndex}]`) || line.includes(`[${String(oppIndex).padStart(2, '0')}]`)) {
                    const match = line.match(/freq:\s*(\d+)\s*kHz/);
                    if (match) return Math.round(parseInt(match[1]) / 1000);
                }
            }
        }
...
// 🔹 STEP 6: Apply Settings (Universal + Dynamic)
async function applyGlobalGpuSettings() {
    const slider = document.getElementById('gpu-opp-slider');
    if (!slider) return;
    
    const sliderPosition = parseInt(slider.value);
    const oppIndex = (detectedOppCount - 1) - sliderPosition;  // Dynamic inversion
    
    try {
        // Save to sdcard (your preference)
        await exec(`mkdir -p /sdcard/MTK_AI_Engine`);
        await exec(`echo "${oppIndex}" > /sdcard/MTK_AI_Engine/global_gpu_opp_index.txt`);
        
        // Apply based on detected path
        if (gpuControlPath?.includes('gpufreqv2')) {
            await exec(`su -c "echo '${oppIndex}' > /proc/gpufreqv2/fix_target_opp_index"`);
        } else if (gpuControlPath?.includes('gpufreq_opp_freq')) {
            // Apply by frequency value for gpufreq
            const targetFreq = gpuFrequencyMap?.[oppIndex] || detectedMaxFreq;
            await exec(`su -c "echo '${targetFreq}000' > /proc/gpufreq/gpufreq_opp_freq"`);
        } else {
            // Fallback: try both            await exec(`su -c "echo '${oppIndex}' > /proc/gpufreqv2/fix_target_opp_index 2>/dev/null || echo '${gpuFrequencyMap?.[oppIndex] || detectedMaxFreq}000' > /proc/gpufreq/gpufreq_opp_freq"`);
        }
        
        // Show frequency in status
        let freqToShow = gpuFrequencyMap?.[oppIndex] || oppIndex;
        updateStatusMsg(`GPU: ${freqToShow} MHz (OPP ${oppIndex})`, "#32D74B");
        
        // Update display
        updateGpuDisplay(oppIndex);
        
    } catch (error) {
        console.error('GPU apply error:', error);
        updateStatusMsg('GPU apply failed', "#FF453A");
    }
}

// 🔹 BONUS: Real-time GPU Monitor (Optional)
async function monitorGpuFrequency() {
    if (!gpuControlPath) return null;
    
    try {
        let currentFreq = null;
        
        if (gpuControlPath.includes('gpufreq_opp_freq')) {
            const freq = await exec(`cat /proc/gpufreq/gpufreq_opp_freq 2>/dev/null`);
            currentFreq = Math.round(parseInt(freq.trim()) / 1000);
        } else if (gpuControlPath.includes('gpufreqv2')) {
            const idx = await exec(`cat /proc/gpufreqv2/fix_target_opp_index 2>/dev/null`);
            const oppIdx = parseInt(idx.trim());
            currentFreq = gpuFrequencyMap?.[oppIdx] || null;
        }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Optional: Auto-refresh GPU display every 2 seconds
    setInterval(async () => {
        const currentFreq = await monitorGpuFrequency();
        if (currentFreq) {
            const display = document.getElementById('gpu-opp-val');
            if (display && !document.getElementById('gpu-opp-slider')?.matches(':active')) {
