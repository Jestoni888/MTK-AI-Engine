...
async function loadSearchCache() {
    try {
        const data = await exec(`cat ${CACHE_FILE} 2>/dev/null`);
        if (data.trim()) {
...
async function startGlobalSearch() {
    ...
    setTimeout(async () => {
        try {
            // ⚠️ SAFE PATHS ONLY
            const safeBasePaths = "/sys/class /sys/devices /sys/ /dev/ /vendor/ /sys/module /proc/sys /proc/thread-self /proc/swpm /proc/ppm /proc/pmsr /proc/net /proc/mtktz /proc/mtk_lpm /proc/gpufreqv2 /proc/fs /proc/driver /proc/cpufreq /proc/asound /system /system_ext /proc/device-tree";
            
            let results = [];

            if (searchName && !searchContent) {
                // MODE: Filename Only
                // ✅ ADDED: -type f (Regular files only)
                // ✅ ADDED: ! -type l (Exclude symbolic links if desired, optional)
                const cmd = `find ${safeBasePaths} -type f -name "*${keyword}*" 2>/dev/null | head -n 100`;
                
                const output = await exec(cmd);
                results = output.split('\n').filter(l => l.trim() !== "");
            } 
            else if (searchContent && !searchName) {
                // MODE: Content Only (grep automatically ignores directories usually, but good to be safe)
                const cmd = `grep -ril --binary-files=without-match "${keyword}" ${safeBasePaths} 2>/dev/null | head -n 100`;
                const output = await exec(cmd);
                results = output.split('\n').filter(l => l.trim() !== "");
            } 
            else {
                ...
                // Name Search (Files Only)
                const nameOut = await exec(`find ${safeBasePaths} -type f -name "*${keyword}*" 2>/dev/null`);
                const nameRes = nameOut.split('\n').filter(l => l.trim() !== "");
                
                // Content Search
                const contentOut = await exec(`grep -ril --binary-files=without-match "${keyword}" ${safeBasePaths} 2>/dev/null`);
                const contentRes = contentOut.split('\n').filter(l => l.trim() !== "");

                // Combine & Deduplicate
                results = [...new Set([...nameRes, ...contentRes])].slice(0, 100);
            }
...
async function saveCacheToSD() {
    try {
        ...
        await exec(`echo "${safeJson}" > ${CACHE_FILE}`);
        document.getElementById('cache-count').textContent = Object.keys(searchCache).length;
...
/* =========================================
   ...
   ========================================= */
let currentEditPath = "";
let originalValue = ""; // Store raw value to detect changes
...
async function openFileEditor(path) {
    ...
    const currentPermDisplay = document.getElementById('editor-current-perm'); // New
    const permInput = document.getElementById('editor-new-perm'); // New
    const permStatus = document.getElementById('perm-status'); // New
    ...
    try {
        // 1. Get File Info (Permissions & Size)
        const statInfo = await exec(`stat -c "%A (%a) | Size: %s bytes" "${path}" 2>/dev/null`);
        
        // Extract just the octal permission (e.g., 644) for display
        const permMatch = statInfo.match(/\((\d+)\)/);
        if (permMatch) {
            currentPermDisplay.textContent = permMatch[1]; // Shows "644"
        ...
        let content = "";
        try {
            content = await exec(`cat "${path}" 2>/dev/null`);
            if (content.includes('\u0000')) {
                status.innerHTML = "⚠️ <span style='color:#FF9F0A'>Binary file detected.</span>";
                const hexDump = await exec(`xxd -p "${path}" 2>/dev/null | fold -w 60`);
                content = "--- HEX DUMP ---\n" + hexDump;
            }
        } catch (e) {
...
async function changeFilePermissions() {
    ...
    try {
        ...
        // Update the displayed permission immediately
        const updatedStat = await exec(`stat -c "%a" "${currentEditPath}" 2>/dev/null`);
...
async function saveFileValue() {
    ...
    try {
        // Special handling if user edited a Hex Dump
        ...
        // Standard Write
        // We use 'tee' which is often more reliable than redirection for special chars
        await exec(`echo "${newValue}" | tee "${currentEditPath}" > /dev/null`);
        ...
        setTimeout(() => {
            btn.textContent = originalText;            closeEditor();
        }, 1500);
