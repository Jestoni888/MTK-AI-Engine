...
•Fix dex2oat compiler killed during off screen, so no worries now even in off screen

•Fixed app loading if no internet connection (Webui Applications tab)
...
⚠️If you want to run apps in background like music then protect it in Never kill lists in Tools tab Ram hibernator because even if ram cleaner is not enabled the background is also restricted, no data received, or idle state. The purpose of this module is to hibernate all apps including system apps + users apps to consistently save battery & lower temperature in normal use
...
•Remove lite mode gaming that cause interference on per app config on CPU frequency & GPU frequency (Tips: Just lower the cpu frequency & GPU frequency on per-app to cap temp not goes on higher)
...
•Added GPU governor selection on per-app also fixed GPU frequency on per-app functions & fallback to global setup in advance GPU frequency/governor setup in Webui if no config found in per-app
...
•swipe to home launcher in order to switch between normal & gaming mode

•added ram hibernator (tools tab)
